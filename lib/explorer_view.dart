...
class ExplorerViewState extends State<ExplorerView> with AutomaticKeepAliveClientMixin<ExplorerView> {
  String currentDir = "/";
  ...
  Future<void> checkAndRequestPermission() async{
    ...
    setState(() {
      haveStoragePermission = permStatus;
    });
  }
  ...
  Future<void> refreshFileList() async{
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error opening $currentDir: $e",),),
      );
      goPreviousPath();
      return;
    }
    ...
    final fileList = combList.where((element) => element.stats.type == FileSystemEntityType.file);
    final folderList = combList.where((element) => element.stats.type == FileSystemEntityType.directory);
  ...
  Future<bool> onWillPop() async{
    if (previousDirs.isEmpty){
      ...
      if (lastBackPressTime is! DateTime ||
          currTime.difference(lastBackPressTime!) > const Duration(seconds: 2)){
  ...
  Widget makeEasyNavBar(){
    ...
    SchedulerBinding.instance.addPostFrameCallback((timeStamp) { navCtrl.jumpTo(navCtrl.position.maxScrollExtent);});
    return outSB;
  }
              ...
              slivers: [
                ...
                SliverFillRemaining(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: const [
                      Text("No Storage Permission", textAlign: TextAlign.center,),
                    ],
                  ) ,
                ),
              ]
